...
def main():
    ...
    print(f"Conflicts: {obs.metadata.get('conflict_files')}")
    print(f"Markers present: {env._has_markers()}")
    print(f"CI passing: {env._is_ci_passing()}")
    ...
    action = CallToolAction(
        tool_name="edit_file",
        arguments={"path": "app/main.py", "content": solve_content}
    )
    obs = env.step(action)
    ...
    print(f"Reward Breakdown: {obs.metadata.get('reward_breakdown')}")
